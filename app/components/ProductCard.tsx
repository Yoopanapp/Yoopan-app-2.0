...
type ProductType = {
  ...
  prix?: number;      // Support rétrocompatible
  magasin?: string;   // Support rétrocompatible
};
...
export default function ProductCard({ product }: { product: ProductType }) {
  ...
  // NOUVEAU : On crée une référence pour l'image qui va s'envoler
  ...
  // Gestion robuste des offres pour l'affichage
  const offersToDisplay = product.offers || [];
  // Si pas d'offres mais un prix direct (cas simple)
    ...
    <div className="relative group flex flex-col h-full">
      ...
      <div ref={imageRef} className="relative aspect-square rounded-[2rem] bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-lg transition-all duration-300 mb-3 overflow-visible">
           ...
           <div className="absolute bottom-3 right-3 z-20">
             {/* NOUVEAU : On passe la ref au bouton pour qu'il sache quelle image lancer */}
              ...
              return (
                <div key={`${offer.magasin}-${index}`} className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-1.5 overflow-hidden">
                    ...
                    <span className={`truncate ${isBest ? 'text-slate-700 dark:text-slate-200 font-medium' : 'text-slate-400'}`}>{offer.magasin}</span>
                  </div>
